...
layer = nn.Linear(M, N, dtype=DTYPE, bias=False)
quantizer = Quantizer()
...
qlayer = Quant3Linear(layer.in_features, layer.out_features, faster=True)
...
for _ in range(COUNT):
    mul = layer(vec)
    torch.cuda.synchronize()
print('float16 linear:', (time.time() - tick) / COUNT)
...
for _ in range(COUNT):
    mul = qlayer(vec)
    torch.cuda.synchronize()
print('3bit linear:', (time.time() - tick) / COUNT)
...
# layer = nn.Linear(M, N, dtype=DTYPE)
