...
__global__ void _nqmv_bias_shift_mulq(uint32_t *W, __half *alpha, __half *input, __half *output,
            int M, int K, int NUM_BITS, int M_TILE_SIZE, int group_size, int batch, int in_size, int out_size){
...
template<int K_TILE_SIZE>
__global__ void _nqmv_bias_shift_mulq_shift1(uint32_t *W, __half *alpha, __half *input, __half *output,
            int M, int K, int NUM_BITS, int M_TILE_SIZE, int group_size, int batch, int in_size, int out_size){
    __shared__ __half lut[K_TILE_SIZE/8][256];
    const int lut_x_size = blockDim.x / (K_TILE_SIZE/8);
...
template<int K_TILE_SIZE>
__global__ void _nqmv_bias_shift_mulq_shift2(uint32_t *W, __half *alpha, __half *input, __half *output,
            int M, int K, int NUM_BITS, int M_TILE_SIZE, int group_size){
    __shared__ __half lut[K_TILE_SIZE/8][256];
    const int lut_x_size = blockDim.x / (K_TILE_SIZE/8);
...
/*nQWeight_fp16 &nqW,*/ __half *input, int num_thraeds, int m_tile_size, int batch, int in_size, int out_size){
    ...
    _nqmv_bias_shift_mulq<k_tile_size><<<grid, block>>>(bWeight, (__half*)alpha, 
            input, output, mSize, kSize, nb, m_tile_size, kSize/num_groups, batch, in_size, out_size);
}

template<int k_tile_size>
inline void _excute_nqmv_bias_shift_mulq_shift1(__half *output,uint32_t*bWeight, __half* alpha,
int mSize, int kSize, int nb,  int num_groups,
/*nQWeight_fp16 &nqW,*/ __half *input, int num_thraeds, int m_tile_size, int batch, int in_size, int out_size){
    dim3 grid(
        div_roundup(mSize, m_tile_size), 
        div_roundup(kSize, k_tile_size)); 
    dim3 block(num_thraeds);
    _nqmv_bias_shift_mulq_shift1<k_tile_size><<<grid, block>>>(bWeight, (__half*)alpha, 
            input, output, mSize, kSize, nb, m_tile_size, kSize/num_groups, batch, in_size, out_size);
}
...
inline auto _get_excute_nqmv_bias_shift_mulq_shift1(int num_thraeds, size_t bits, size_t k_tile_idx){
    void (*funcs[])(__half *output, uint32_t*bWeight, __half* alpha, 
int mSize, int kSize, int nb,  int num_groups, __half *input, int num_thraeds, int m_tile_size, int batch, int in_size, int out_size)= {
        _excute_nqmv_bias_shift_mulq_shift1< 32*1>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*2>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*3>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*4>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*5>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*6>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*7>, 
        _excute_nqmv_bias_shift_mulq_shift1< 32*8>, 
    };
    return funcs[k_tile_idx];
}
...
int mSize, int kSize, int nb,  int num_groups, __half *input, int num_thraeds, int m_tile_size, int batch, int in_size, int out_size) = _get_excute_nqmv_bias_shift_mulq(num_thraeds, nb, k_tile_idx);
    func(output, bWeight, alpha, mSize, kSize, nb, num_groups, input, num_thraeds, m_tile_size, batch, in_size, out_size);
}

inline void nqmv_bias_shift_mulq_shift1(__half *output, uint32_t*bWeight, __half* alpha,
int mSize, int kSize, int nb,  int num_groups, __half *input, int algo, int batch, int in_size, int out_size){
    int k_tile_idx   =     0;
    int m_tile_size  =  2048;
    int num_thraeds  =   256;
    void  (*func)(__half *output, uint32_t*bWeight, __half* alpha,
int mSize, int kSize, int nb,  int num_groups, __half *input, int num_thraeds, int m_tile_size, int batch, int in_size, int out_size) = _get_excute_nqmv_bias_shift_mulq_shift1(num_thraeds, nb, k_tile_idx);
    func(output, bWeight, alpha, mSize, kSize, nb, num_groups, input, num_thraeds, m_tile_size, batch, in_size, out_size);
}
